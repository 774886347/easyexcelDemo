package com.wings.easyexcel.utils.excel;
...
import com.wings.easyexcel.utils.common.BeanConvert;
...
public class ExcelUtil {
     ...
     * @return 返回实体列表(需转换)
     */
    public static <T> List<T> readExcel(MultipartFile excel, Class<T> clazz, int sheetNo, ReadListener readListener) {

        ExcelListener excelListener = new ExcelListener();

        ExcelReader excelReader = getReader(excel,clazz,readListener);
        if (excelReader == null) {
     ...
     * @param readListener
     */
    private static <T> ExcelReader getReader(MultipartFile excel, Class<T> clazz, ReadListener readListener) {
        ...
        try {
            ...
            ExcelReader excelReader = EasyExcel.read(inputStream, clazz, readListener).build();
